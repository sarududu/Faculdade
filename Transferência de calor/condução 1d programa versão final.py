...
def rodar_simulacao(root, a, k, Q, length, total_time, nodes, u_0, u_n, temperatura_inicial, axis, canvas, pcm, parar_simulacao_callback):
        ...
        if simulacao_ativa:  # Verifica se a simulação ainda está ativa
            if counter < total_time:
                ...
                for i in range(1, nodes - 1):
                    # Adiciona o termo de geração de calor Q e condutividade térmica k
                    u[i] = (
                        dt * a * (w[i - 1] - 2 * w[i] + w[i + 1]) / dx**2
                        + dt * Q / k
                        + w[i]
                    )
...
def resultado_final(difusividade_entry, condutividade_entry, geracao_calor_entry, comprimento_entry, tempo_entry, u_inicio_entry, u_fim_entry, temperatura_inicial_entry, axis, canvas, pcm):
    try:
        # Chamada para os valores da simulação
        a = float(difusividade_entry.get())
        k = float(condutividade_entry.get())
        Q = float(geracao_calor_entry.get())
        length = float(comprimento_entry.get())
        total_time = float(tempo_entry.get())
        nodes = int(100)
        u_0 = float(u_inicio_entry.get())
        u_n = float(u_fim_entry.get())
        temperatura_inicial = float(temperatura_inicial_entry.get())

        # Verificação simples para valores negativos ou 0
        if a <= 0 or k <= 0 or length <= 0 or total_time <= 0 or nodes <= 0 or u_0 < 0 or u_n < 0 or temperatura_inicial < 0:
        ...
        x = np.linspace(0, length, nodes)
        u = np.zeros(nodes) + temperatura_inicial
        u[0] = u_0
        u[-1] = u_n
        ...
        # Atualizar o gráfico com o estado final
        u_final_2d = np.vstack([u, u])  # Transformar para 2D
        pcm.set_array(u_final_2d.ravel())
        axis.set_title(f"Estado final: t = {total_time:.2f} s")
        canvas.draw()
...
def abrir_interface():
    def iniciar_simulacao():
        try:
            ...
            rodar_simulacao(root, a, k, Q, length, total_time, nodes, u_0, u_n, temperatura_inicial, axis, canvas, pcm, parar_simulacao)
    ...
    def parar_simulacao():
        ...
        parar_simulacao_button.config(state=tk.DISABLED)
    ...
    ttk.Label(config_frame, text="Condutividade térmica (k):").grid(row=1, column=0, sticky=tk.W, padx=10, pady=5)
    condutividade_entry = ttk.Entry(config_frame)
    condutividade_entry.insert(0, "200")
    condutividade_entry.grid(row=1, column=1, padx=10, pady=5, sticky=tk.EW)

    # Geração de calor
    ttk.Label(config_frame, text="Geração de calor (Q):").grid(row=2, column=0, sticky=tk.W, padx=10, pady=5)
    geracao_calor_entry = ttk.Entry(config_frame)
    geracao_calor_entry.insert(0, "0")
    geracao_calor_entry.grid(row=2, column=1, padx=10, pady=5, sticky=tk.EW)

    # Comprimento da placa
    ttk.Label(config_frame, text="Comprimento da placa (mm):").grid(row=3, column=0, sticky=tk.W, padx=10, pady=5)
    comprimento_entry = ttk.Entry(config_frame)
    comprimento_entry.insert(0, "100")
    comprimento_entry.grid(row=3, column=1, padx=10, pady=5, sticky=tk.EW)

    # Tempo total da simulação
    ttk.Label(config_frame, text="Tempo total da simulação (s):").grid(row=4, column=0, sticky=tk.W, padx=10, pady=5)
    tempo_entry = ttk.Entry(config_frame)
    tempo_entry.insert(0, "50")
    tempo_entry.grid(row=4, column=1, padx=10, pady=5, sticky=tk.EW)

    # Temperatura inicial da placa
    ttk.Label(config_frame, text="Temperatura inicial da placa (\u00B0C):").grid(row=5, column=0, sticky=tk.W, padx=10, pady=5)
    temperatura_inicial_entry = ttk.Entry(config_frame)
    temperatura_inicial_entry.insert(0, "20")
    temperatura_inicial_entry.grid(row=5, column=1, padx=10, pady=5, sticky=tk.EW)

    # Condição de contorno inicial (u[0])
    ttk.Label(config_frame, text="Temperatura extremidade direita da placa:").grid(row=6, column=0, sticky=tk.W, padx=10, pady=5)
    u_inicio_entry = ttk.Entry(config_frame)
    u_inicio_entry.insert(0, "100")
    u_inicio_entry.grid(row=6, column=1, padx=10, pady=5, sticky=tk.EW)

    # Condição de contorno final (u[-1])
    ttk.Label(config_frame, text="Temperatura extremidade esquerda da placa:").grid(row=7, column=0, sticky=tk.W, padx=10, pady=5)
    u_fim_entry = ttk.Entry(config_frame)
    u_fim_entry.insert(0, "0")
    u_fim_entry.grid(row=7, column=1, padx=10, pady=5, sticky=tk.EW)
    ...
    iniciar_button.grid(row=8, column=0, columnspan=2, pady=20)

    # Botão para parar a simulação
    parar_simulacao_button = ttk.Button(config_frame, text="Parar Simulação", command=parar_simulacao)
    parar_simulacao_button.grid(row=9, column=0, columnspan=2, pady=20)
    parar_simulacao_button.config(state=tk.DISABLED)

    # Botão para mostrar o resultado final
    resultado_final_button = ttk.Button(config_frame, text="Resultado Final", command=lambda: resultado_final(difusividade_entry, condutividade_entry, geracao_calor_entry, comprimento_entry, tempo_entry, u_inicio_entry, u_fim_entry, temperatura_inicial_entry, axis, canvas, pcm))
    resultado_final_button.grid(row=10, column=0, columnspan=2, pady=20)
    ...
    u_initial[0] = 100
    u_initial[-1] = 0
    u_initial_2d = np.vstack([u_initial, u_initial])
