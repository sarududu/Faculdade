...
def rodar_simulacao(root, a, length, total_time, nodes, u_0, u_n, temperatura_inicial, axis, canvas, pcm, parar_simulacao_callback):
    ...
    y = np.array([0, 1])  # Gráfico bidimensional
...
def resultado_final(difusividade_entry, comprimento_entry, tempo_entry, u_inicio_entry, u_fim_entry, temperatura_inicial_entry, axis, canvas, pcm):
    try:
...
def abrir_interface():
    ...
    ttk.Label(config_frame, text="Difusividade térmica (mm²/s) (α):").grid(row=0, column=0, sticky=tk.W, padx=10, pady=5)
    ...
    resultado_final_button = ttk.Button(config_frame, text="Resultado Final", command=lambda: resultado_final(difusividade_entry, comprimento_entry, tempo_entry, u_inicio_entry, u_fim_entry, temperatura_inicial_entry, axis, canvas, pcm))
    resultado_final_button.grid(row=8, column=0, columnspan=2, pady=20)
